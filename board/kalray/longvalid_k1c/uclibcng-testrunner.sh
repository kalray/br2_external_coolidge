...
nskip=0
npass=0
while read expected_status expected_ret tst_src_name binary_name subdir cmd; do
	...
	test $status = $expected_status || {
		echo "$binary_name: expected test $expected_status but is $status"
		testsuite_status="FAIL"
	}
done <uclibcng-testrunner.in

echo Status: $testsuite_status
